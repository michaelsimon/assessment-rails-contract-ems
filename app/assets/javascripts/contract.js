...
var contractDetail = function() {
  ...
  $.get(`/contracts/${contract_id}/details`, function(){}).done(function(data){
    ...
    allDocuments.forEach(function(document){
      ...
      $('#documents').append(`<tr data-document-id="${documentObject.id}"><td><a href="/contracts/${contractObject.id}/documents/${documentObject.id}">${documentObject.name}</a></td><td>${documentObject.description}</td><td>${documentObject.rights}</td></tr>`);
    });
